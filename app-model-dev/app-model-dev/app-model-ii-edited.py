...
cur.execute("CREATE TABLE IF NOT EXISTS chats(id serial PRIMARY KEY, name varchar, prompt varchar, output varchar, datetime varchar)")
con.commit()
...
with columnB:
    ...
    if prompt:
        ...
        SQL = "INSERT INTO chats (name, prompt, output, datetime) VALUES(%s, %s, %s, %s);"
        ...
        with st.expander(f"See Previous Conversation for {input_name}"):
            cur.execute(f"""
                        ...
                        """)
            for id, name, prompt, output, datetime in cur.fetchall():
                message = st.chat_message("user")
                message.write(f":blue[{name}]: {prompt}")
                message.caption(f"{datetime}")
                message = st.chat_message("assistant")
                message.write(output)
                message.caption(f"{datetime}")
    else:
